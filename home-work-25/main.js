...
function createDomElement(tagName, textContent, container) {
  if (!tagName || !container) {
    console.error('Необхідно передати назву тега та контейнер!');
    return null;
  }

  const element = document.createElement(tagName); // створюємо елемент
  element.textContent = textContent; // додаємо текст
  container.appendChild(element); // додаємо у контейнер
  console.log(`Створено <${tagName}> з текстом: "${textContent}"`);
  return element;
}
...
function setUserInfoCookie(key, value) {
  if (!key || !value) {
    console.error('Потрібно передати key та value!');
    return;
  }

  const encodedValue = encodeURIComponent(`${key}=${value}`);
  const expires = new Date(Date.now() + 10 * 1000).toUTCString(); // 10 секунд
  document.cookie = `userInfo=${encodedValue}; expires=${expires}; path=/`;

  console.log(`Cookie userInfo збережено: ${key}=${value} (10 секунд)`);
}
...
function saveUserInfo(key, value) {
  if (!key) {
    console.error('Необхідно передати ключ для збереження!');
    return;
  }
...
function getUserInfo(key) {
  if (!key) {
    console.error('Необхідно передати ключ для отримання даних!');
    return null;
  }
